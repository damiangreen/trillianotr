...
HWND CSMPStartDlg::Create(HWND hWndParent, LPARAM dwInitParam)
{
	conn = reinterpret_cast<ConnContext*>(dwInitParam);
	hWnd = CDialogImpl<CSMPStartDlg>::Create(hWndParent, 0);

	SMPQuestion.Attach(::GetDlgItem(hWnd, IDC_SMPQUESTIONEDIT));
	SMPAnswer.Attach(::GetDlgItem(hWnd, IDC_SMPANSWEREDIT));

	return hWnd;
}

LRESULT CSMPStartDlg::OnFinishDlg(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	CString answer;
	CString question;
	ShowWindow(SW_HIDE);
	...
	/* Get our data */
	SMPAnswer.GetWindowTextA(answer);
	SMPQuestion.GetWindowTextA(question);
	/* Make the SMP request */
	totrPluginStartSMP(conn, question, answer, answer.GetLength());
