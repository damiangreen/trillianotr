...
HWND CAboutDlg::Create(HWND hWndParent, LPARAM dwInitParam)
{
	hWnd = CDialogPrefs::Create(hWndParent, dwInitParam);
	hVersion.Attach(::GetDlgItem(hWnd, IDC_VERSION_TEXT));
	hVersion.SetWindowText(totr_config.GetVersion());

//	hSF.Attach(::GetDlgItem(hWnd, IDC_SFCHK));

	return hWnd;
}

LRESULT CAboutDlg::OnSFCheck(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	ShellExecute(NULL, "open", "https://sourceforge.net/projects/trillianotr",NULL, NULL, SW_SHOWNORMAL);
	return 0;
}

LRESULT CAboutDlg::OnGPL(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	ShellExecute(NULL, "open", "http://www.gnu.org/licenses/",NULL, NULL, SW_SHOWNORMAL);
	return 0;
}